...
class RemoteHost(Host):
    ...
    def is_host_type(cls, identifier, configholder):
        ...
        return can_reach_remote_dir(expanded_path)
    ...
    def add_parser(cls, subparser_container):
        ...
        remotehost.add_argument('host', help='specifies what host you wish to target, host format is the format of hostname in ssh')
        remotehost.add_argument('--host-path', metavar="path", default=default_host_path, help=f'specifies what directory on the host to target for repos (default: {default_host_path}).')
        return remotehost
