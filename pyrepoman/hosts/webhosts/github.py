...
class GitHub(WebHost):
    ...
    _HELP_DESC = 'a popular web hosting for git repos'
    ...
    def _modify_parser(cls, parser):
        ...
        subparser_github_container = parser.add_subparsers(title='available repo-owner-types', metavar='repo-owner-type [options ...]', dest='repo_owner_type')
    ...
    def _get_user_repo_names_and_locations(self):
        ...
        url = "https://api.github.com/user/repos" if self.repo_owner_type == 'own' else f"https://api.github.com/users/{self.username}/repos"
        try:
            auth = GitHubAuth(getattr(self, 'api_token')) if self.repo_owner_type == 'own' else None
            ...
            print(f"Error: no api token exists in the configuration file for {type(self).__name__.lower()}")
            raise SystemExit()
